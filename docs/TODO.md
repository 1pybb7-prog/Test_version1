...
- [x] 환경변수 설정 (`.env.local` 파일) ✅
  ...
  - [x] `NEXT_PUBLIC_TOUR_PET_API_KEY` (한국관광공사 반려동물 동반여행 API) ✅
...
- [x] `lib/types/tour.ts` - 관광지 타입 정의 ✅
  ...
  - [x] `TourDetail` 인터페이스 ✅
  - [x] `TourIntro` 인터페이스 ✅
  - [x] `PetTourInfo` 인터페이스 (MVP 2.5) - 반려동물 동반 정보 타입 ✅
...
- [x] `lib/api/tour-api.ts` - 한국관광공사 API 호출 함수들 ✅
  ...
  - [x] `getDetailPetTour` - 반려동물 동반 정보 조회 (MVP 2.5) ✅
...
- [x] 반려동물 필터 추가 ✅
  ...
  - [ ] 반려동물 종류별 필터 (개, 고양이 등) - API 응답에 종류 정보가 없을 수 있어 일단 제외
  - [x] 실내/실외 동반 가능 여부 필터 ✅
- [x] `components/TourFilter.tsx`에 반려동물 필터 UI 추가 ✅
- [x] `hooks/useTourFilter.ts`에 반려동물 필터 상태 추가 ✅
- [x] `hooks/usePetTourFilter.ts` 생성 - 반려동물 필터링 로직 ✅
- [x] 필터 로직 연동 ✅
- [x] 반려동물 가능 관광지 카드에 🐾 아이콘 표시 ✅
- [x] 크기 제한 뱃지 표시 (예: "소형", "중형", "대형") ✅
- [x] 홈페이지에 반려동물 필터 통합 ✅
- [ ] 페이지 확인 및 UX 개선 (사용자 테스트 필요)
...
- [x] `components/tour-detail/TourDetailPetTour.tsx` 생성 ✅
  - [x] Guideline 준수: PascalCase 네이밍 ✅
  - [x] `detailPetTour2` API 연동 ✅
  - [x] 반려동물 동반 가능 여부 표시 ✅
  - [x] 반려동물 크기/종류 제한 정보 표시 ✅
  - [x] 추가 요금 및 시설 정보 표시 ✅
  - [x] 주차장 정보 (반려동물 하차 공간) ✅
  - [x] 기타 반려동물 정보 표시 ✅
  - [x] 아이콘 및 뱃지 디자인 (🐾 아이콘) ✅
  - [x] Spacing-First 정책 준수 ✅
- [x] `actions/get-tour-pet.ts` 생성 - Server Action ✅
- [x] `hooks/useTourPet.ts` 생성 - React Query 훅 ✅
- [x] 상세페이지에 반려동물 정보 섹션 추가 ✅
- [ ] 페이지 확인 및 스타일링 (반응형 검증) (사용자 테스트 필요)
